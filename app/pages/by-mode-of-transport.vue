...
import type { ModeOfTransportItem } from '~/stores/modeOfTransport'
import type { DataTableHeader } from 'vuetify'

const store = useModeOfTransportStore()
...
const filteredItems = ref<ModeOfTransportItem[]>([])
...
const editingItem = ref<ModeOfTransportItem | null>(null)
const form = ref({
  period: '',
  description: '',
  netWeight: 0,
...
function matchSearch(item: ModeOfTransportItem, q: string): boolean {
  if (!q.trim()) return true
  const lower = q.trim().toLowerCase()
  const s = (v: unknown) => String(v).toLowerCase()
  return (
    s(item.period).includes(lower) ||
    s(item.description).includes(lower) ||
    s(item.netWeight).includes(lower) ||
...
/** Period, Description, Net Weight (tons), Value (sortable), Actions */
const tableHeaders = computed<DataTableHeader[]>(() => [
  { title: t('common.table.period'), key: 'period', sortable: true },
  { title: t('common.table.description'), key: 'description', sortable: true },
  { title: t('common.table.netWeightTons'), key: 'netWeight', sortable: true },
...
const barChartData = computed(() => ({
  ...
  datasets: [
    {
      label: t('common.chart.modeOfTransportValue'),
...
function openEdit(item: unknown) {
  const row = item as ModeOfTransportItem | undefined
  if (!row?.id) return
...
async function remove(item: unknown) {
  const row = item as ModeOfTransportItem
...
const summaryUniqueDescriptions = computed(() => {
  const set = new Set(searchFilteredItems.value.map((x) => x.description))
  ...
  <div class="flex min-h-0 min-w-0 flex-col">
    ...
    <div class="mb-2 min-w-0 shrink-0">
      <div class="grid w-full grid-cols-2 gap-2 md:grid-cols-4">
        ...
        <SummaryCard :title="t('common.table.description')" :value="summaryUniqueDescriptions" icon="mdi-truck-delivery" />
    ...
    <!-- Table: Period, Description, Net Weight (tons), Value, Actions -->
    <div class="by-mode-of-transport-table-wrapper min-w-0 flex-1 overflow-x-auto">
      <DataTableCrud
    ...
    <!-- Add/Edit dialog -->
    <AppCrudDialog v-model="dialog" entity-name="Mode of Transport" :is-edit="!!editingItem" :max-width="560" @save="save">
      <div class="crud-form-scroll overflow-x-auto pt-2 -webkit-overflow-scrolling-touch px-1">
        <div class="min-w-[280px] space-y-3 pr-2">
          ...
          <v-text-field
            ...
            :label="t('common.table.netWeightTons')"
            type="number"
            density="compact"
            hide-details
          />
    ...
    <AppAnalyst
      ...
      download-filename-prefix="by-mode-of-transport"
...
.by-mode-of-transport-table-wrapper :deep(.v-data-table),
.by-mode-of-transport-table-wrapper :deep(.v-table) {
  font-size: 0.75rem;
}
