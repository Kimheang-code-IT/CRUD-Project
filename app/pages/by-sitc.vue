...
type SitcItem = ISitcItem

const store = useSitcStore()
...
const filteredItems = ref<SitcItem[]>([])
...
const editingItem = ref<SitcItem | null>(null)
const form = ref({
  period: '',
  sitcCode: '',
  sitcDescription: '',
...
function matchSearch(item: SitcItem, q: string): boolean {
  if (!q.trim()) return true
  const lower = q.trim().toLowerCase()
  const s = (v: unknown) => String(v).toLowerCase()
  return (
    s(item.period).includes(lower) ||
    s(item.sitcCode).includes(lower) ||
    s(item.sitcDescription).includes(lower) ||
...
/** Period, SITC Code, SITC Description, Value (sortable), Actions */
const tableHeaders = computed<DataTableHeader[]>(() => [
  { title: t('common.table.period'), key: 'period', sortable: true },
  { title: t('common.table.sitcCode'), key: 'sitcCode', sortable: true },
  { title: t('common.table.sitcDescription'), key: 'sitcDescription', sortable: true },
...
const barChartData = computed(() => ({
  ...
  datasets: [
    {
      label: t('common.chart.sitcValue'),
...
function openEdit(item: unknown) {
  const row = item as SitcItem | undefined
  if (!row?.id) return
  ...
  form.value = {
    period: raw.period,
    sitcCode: raw.sitcCode,
    sitcDescription: raw.sitcDescription,
    value: raw.value,
  }
  dialog.value = true
}
...
const summaryUniqueSitcCodes = computed(() => {
  const set = new Set(searchFilteredItems.value.map((x) => x.sitcCode))
  ...
  <div class="flex min-h-0 min-w-0 flex-col">
    ...
    <div class="mb-2 min-w-0 shrink-0">
      <div class="grid w-full grid-cols-2 gap-2 md:grid-cols-4">
        ...
        <SummaryCard :title="t('common.table.sitcCode')" :value="summaryUniqueSitcCodes" icon="mdi-barcode" />
    ...
    <!-- Table: Period, SITC Code, SITC Description, Value, Actions -->
    <div class="by-sitc-table-wrapper min-w-0 flex-1 overflow-x-auto">
      <DataTableCrud
    ...
    <!-- Add/Edit dialog -->
    <AppCrudDialog v-model="dialog" entity-name="SITC" :is-edit="!!editingItem" :max-width="560" @save="save">
      <div class="crud-form-scroll overflow-x-auto pt-2 -webkit-overflow-scrolling-touch px-1">
        <div class="min-w-[280px] space-y-3 pr-2">
          ...
          />
          <v-text-field
            v-model="form.sitcCode"
            :label="t('common.table.sitcCode')"
            density="compact"
            hide-details
          />
          <v-text-field
            v-model="form.sitcDescription"
            :label="t('common.table.sitcDescription')"
            density="compact"
            hide-details
          />
          <v-text-field
    ...
    <AppAnalyst
      ...
      download-filename-prefix="by-sitc"
...
.by-sitc-table-wrapper :deep(.v-data-table),
.by-sitc-table-wrapper :deep(.v-table) {
  font-size: 0.75rem;
}
