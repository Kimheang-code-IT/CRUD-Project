...
import type { HsCodeItem } from '~/stores/hsCode'
import type { DataTableHeader } from 'vuetify'

const store = useHsCodeStore()
...
const filteredItems = ref<HsCodeItem[]>([])
...
const editingItem = ref<HsCodeItem | null>(null)
const form = ref({
  period: '',
  hsCode: '',
  hsDescription: '',
...
function matchSearch(item: HsCodeItem, q: string): boolean {
  if (!q.trim()) return true
  const lower = q.trim().toLowerCase()
  const s = (v: unknown) => String(v).toLowerCase()
  return (
    s(item.period).includes(lower) ||
    s(item.hsCode).includes(lower) ||
    s(item.hsDescription).includes(lower) ||
...
/** Only: Product Code, HS Code, HS Code Description, Action (sortable) */
const tableHeaders = computed<DataTableHeader[]>(() => [
  { title: t('common.table.productCode'), key: 'period', sortable: true },
  { title: t('common.table.hsCode'), key: 'hsCode', sortable: true },
  { title: t('common.table.hsCodeDescription'), key: 'hsDescription', sortable: true },
  { title: t('common.table.actions'), key: 'actions', sortable: true },
])
...
const barChartData = computed(() => ({
  ...
  datasets: [
    {
      label: t('common.chart.hsCodeValue'),
...
function openEdit(item: unknown) {
  const row = item as HsCodeItem | undefined
  if (!row?.id) return
  ...
  form.value = {
    period: raw.period,
    hsCode: raw.hsCode,
    hsDescription: raw.hsDescription,
    value: raw.value,
  }
  dialog.value = true
}
...
const summaryUniqueHsCodes = computed(() => {
  const set = new Set(searchFilteredItems.value.map((x) => x.hsCode))
  ...
  <div class="flex min-h-0 min-w-0 flex-col">
    ...
    <div class="mb-2 min-w-0 shrink-0">
      <div class="grid w-full grid-cols-2 gap-2 md:grid-cols-4">
        ...
        <SummaryCard :title="t('common.table.productCode')" :value="summaryRecordCount" icon="mdi-format-list-numbered" />
        <SummaryCard :title="t('common.table.hsCode')" :value="summaryUniqueHsCodes" icon="mdi-barcode" />
    ...
    <!-- Table: only Product Code, HS Code, HS Code Description, Action -->
    <div class="by-hs-code-table-wrapper min-w-0 flex-1 overflow-x-auto">
      <DataTableCrud
    ...
    <AppCrudDialog v-model="dialog" entity-name="HS Code" :is-edit="!!editingItem" :max-width="560" @save="save">
      <div class="crud-form-scroll overflow-x-auto pb-2 -webkit-overflow-scrolling-touch px-1">
        <div class="min-w-[280px] space-y-3 pr-2">
          <v-text-field
            v-model="form.period"
            :label="t('common.table.productCode')"
            density="compact"
            hide-details
          />
          <v-text-field
            v-model="form.hsCode"
            :label="t('common.table.hsCode')"
            density="compact"
            hide-details
          />
          <v-text-field
            v-model="form.hsDescription"
            :label="t('common.table.hsCodeDescription')"
            density="compact"
            hide-details
          />
          <v-text-field
    ...
    <AppAnalyst
      ...
      download-filename-prefix="by-hs-code"
...
.by-hs-code-table-wrapper :deep(.v-data-table),
.by-hs-code-table-wrapper :deep(.v-table) {
  font-size: 0.75rem;
}
